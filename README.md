...
* Prettier를 실행하면 모든 탭은 공백으로 전환될 뿐만 아니라 들여쓰기, 따음표 형식과 같은 요
...
* 이 지점에 도달한다면 React의 장점을 최대한 활용하기 위해 React 프레임원크로 마이그레이션
하는 것을 권장합니다.
...
* 스트리밍 서버 측 렌더링(SSR)은 서버에서 페이지를 렌더링하고 완전히 렌더링된 페이지를 클라
이언트로 전송합니다.
...
* React Server Components(RSC)를 사용하면 빌드 타임, 서버 전용, 인터랙티브 컴포넌트를 단일 React 트리에 포함할 수 있습니다.
  * RSC는 성능을 향상시킬 수 있지만, 현재 설정 및 유지 관리에 대한 전문 지식이 필요합니다.
  * Parcel의 RSC 예시를 참조하세요.


* 렌더링 전략은 라우터와 통합되어야 프레임워크로 빌드된 앱이 경로별로 렌더링 전략을 선택할 수
있습니다.
